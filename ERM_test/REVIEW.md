...
| COVID | The source tags only three cases (IDs 200887, 200850 and 201306); only 200850 is an EU-27 case, while the other two are coded World. The dashboard now also includes explicit COVID/coronavirus/SARS-CoV-2 mentions in descriptions: 1,133 cases overall, including 743 in the default EU-27 selection. Of these, 1,130 are description matches without a source tag. These are mentions, not a causal classification. |
...
## Chart headings and COVID theme

Removed the repeated category lines above card titles, including Geography, Time series, Breakdown and Announcements, and their subtitle text. Card titles and controls remain.

The COVID undercount came from relying on sparsely populated source tags. Published examples such as [Domino's Pizza, factsheet 103627](https://apps.eurofound.europa.eu/restructuring-events/detail/103627) and [Camile Thai, factsheet 103579](https://apps.eurofound.europa.eu/restructuring-events/detail/103579) explicitly describe COVID-related demand but have no COVID tag in the export. The filter now combines source tags with explicit disease mentions in `Description` and `Description_lower`; its displayed definition explains that mentions do not establish causation. Source flags containing the label `Covid-19` are recognised as well as boolean flags and delimited markers. Audit totals distinguish source-tagged cases from description-only matches. Setting `covidDescriptionMatches: false` restores tag-only classification.
